...
def find_largest_quad(bin_img: np.ndarray) -> Tuple[np.ndarray, str]:
    ...
    h, w = bin_img.shape[:2]
    # at least 0.1% of image or 1000 px
...
def detect_corners_blackbg(img_bgr: np.ndarray, resize_width: Optional[int]) -> Tuple[np.ndarray, str, float]:
    ...
    if resize_width is not None and resize_width > 0 and img_bgr.shape[1] > resize_width:
        ...
        img_proc = cv2.resize(img_bgr, (int(
            img_bgr.shape[1] * scale), int(img_bgr.shape[0] * scale)), cv2.INTER_AREA)
    else:
        scale = 1.0
        img_proc = img_bgr
...
def open_camera(cam_index: int) -> cv2.VideoCapture:
    cap = cv2.VideoCapture(
        cam_index, cv2.CAP_DSHOW) if os.name == "nt" else cv2.VideoCapture(cam_index)
    if not cap.isOpened():
...
def capture_image_main():
    ...
    ap.add_argument("--out", default="part_2_maze_solution/maze_corners_overlay.png",
                    help="Path to save overlay visualization")
    ap.add_argument("--width", type=int, default=1200,
                    help="Processing resize width (0 to disable)")
    ap.add_argument("--expand", type=float, default=10.0,
                    help="Outward expansion in pixels (negative shrinks inward)")
    ap.add_argument("--json-out", type=str, default="part_2_maze_solution/camera_capture_corners.json",
                    help="Corners JSON output path (default: camera_capture_corners.json)")
    ap.add_argument("--save-raw", type=str, default="part_2_maze_solution/camera_capture.png",
    ...
    while True:
        ...
        if key in (27, ord('q')):
            ...
            if frame is None:
                print("No frame captured. Exiting.")
                break
            raw_path, overlay_path, json_path = run_detection_and_save(
                frame, args)
        else:
            continue
...
def parse_target_size(s: str) -> Tuple[int, int]:
    try:
        ...
        raise ValueError(
            "Invalid --target-size. Use WIDTHxHEIGHT (e.g., 1200x800).")
...
def read_corners_from_json(json_path: str) -> Tuple[np.ndarray, str]:
    ...
    if isinstance(data, dict):
        ...
        # Or root-level TL/TR/BR/BL
            ...
            pts = np.array([data["TL"], data["TR"], data["BR"],
                           data["BL"]], dtype=np.float32)
        # Or root-level list
        elif isinstance(data, list) and len(data) == 4:
            pts = np.array(data, dtype=np.float32)
        else:
            raise ValueError(
                "JSON doesn't contain recognizable corners format.")
        ...
        raise ValueError(
            "JSON root must be an object or a list of 4 [x,y] pairs.")
...
def maze_warp_from_json():
    ap = argparse.ArgumentParser()
    ap.add_argument(
        "--json", default="part_2_maze_solution/camera_capture_corners.json", help="Path to corners JSON")
    ap.add_argument(
        "--image", help="Optional override for image path; defaults to 'input' inside JSON, relative to JSON file dir")
    ap.add_argument("--out", default="part_2_maze_solution/maze_warp.png",
                    help="Output warped image path")
    ap.add_argument(
        "--target-size", help="Force output size as WIDTHxHEIGHT (e.g., 1200x800). If omitted, inferred from quad")
    ...
    if args.image:
        ...
        if not img_from_json:
            print(
                "No image path provided via --image and none found in JSON 'input' field.", file=sys.stderr)
            sys.exit(2)
...
def detect_circles_and_overlay(img_bgr, out_path):
    ...
    lower_green = np.array([30, 30, 30])   # Much lower thresholds
    upper_green = np.array([90, 255, 255])  # Wider hue range
    mask_green = cv2.inRange(hsv, lower_green, upper_green)

    # --- VERY permissive Red range ---
    lower_red1 = np.array([0, 30, 30])      # Much lower thresholds
    ...
    for mask, color_name in [(mask_green, "green"), (mask_red, "red")]:
        ...
        for cnt in contours:
            ...
            if area < 100:  # Even lower threshold
                continue
            ...
            cv2.putText(frame, color_name, (u + 10, v - 10),
            ...
            print(
                f"    ✓ Accepted {color_name} circle at ({u}, {v}) with area {area:.1f}")
    ...
    print(
        f"  → TOTAL Detected {len(results)} circles: {[r['color'] for r in results]}")
    return results
...
def maze_circles_and_grid():
    ...
    walls_mask = morph(walls_mask, k_open=max(
        0, args.open), k_close=max(0, args.close))
    ...
    for gy in range(gh):
        ...
        for gx in range(gw):
            ...
            cells.append({"row": int(gy), "col": int(gx), "value": int(
                value), "center_px": [int(cx), int(cy)]})
    ...
    for circle in circles_info:
        ...
        # Clamp to image size and then to grid dimensions
        ...
        for gy in range(gy_min, gy_max + 1):
            for gx in range(gx_min, gx_max + 1):
                # We update the value to 1 (open) in both the matrix and the list
                values_mat[gy, gx] = 1

                # Find the corresponding cell in the 'cells' list using its index
                cell_index = gy * gw + gx
    ...
    print(
        f"Circles overlay saved to: {args.circles_overlay_out} ({len(circles_info)} circles)")
...
def parse_start_end(
    ...
    def parse_point(arg, default_rc):
        ...
        raise ValueError(
            f"Invalid start/end value: {arg}. Use 'green', 'red', or 'r,c'.")
...
def draw_path_on_image(
    ...
    rx, ry = end_circle_px
    draw.ellipse((gx - g_rad, gy - g_rad, gx + g_rad, gy + g_rad),
                 outline=(0, 255, 0, 255), width=3)
    draw.ellipse((rx - r_rad, ry - r_rad, rx + r_rad, ry + r_rad),
...
def write_path_json(
    ...
    out = {
        ...
        # moves between cells (excludes dot-to-cell hops)
...
def solve_maze():
    ...
    ap.add_argument("--json_path", default="part_2_maze_solution/result.json",
...
def read_corners_json(path: str) -> Tuple[np.ndarray, str]:
    ...
    if isinstance(data, dict):
        ...
        if not isinstance(img_path, str) or not img_path:
            raise ValueError(
                "Corners JSON must include original image path in 'input'.")
    else:
...
def collect_polyline_pixels(
    ...
    if mode == "cells" or (mode == "auto" and not has_pixels() and has_cells()):
        ...
        return polypart_2_maze_solution/result.json  # type: ignore

    raise ValueError(
        "Path JSON must contain either 'path_pixels' or 'path_cells'.")


def unwarped_and_overlay_path():
    ap = argparse.ArgumentParser(
        description="Unwarp path points and overlay them on the original image.")
    ap.add_argument("--corners_json", default="part_2_maze_solution/camera_capture_corners.json",
                    help="Corners JSON used for warping.")
    ap.add_argument("--path_json", default="part_2_maze_solution/solution_path_points.json",
                    help="Path JSON produced by solver.")
    ap.add_argument("--warped-image", default="part_2_maze_solution/maze_warp.png",
                    help="Path to the warped image used when solving (to read W,H). If omitted, tries 'maze_warp.png'.")
    ...
    ap.add_argument("--out-image", default="part_2_maze_solution/original_with_path.png",
                    help="Output overlay on the ORIGINAL image.")
    ap.add_argument("--out-json", default="part_2_maze_solution/solution_path_points_unwarped.json",
    ...
    src_rect = np.array([[0, 0], [W - 1, 0], [W - 1, H - 1],
                        [0, H - 1]], dtype=np.float32)   # warped space
    # original image space
    dst_quad = corners.astype(np.float32)
    M_inv = cv2.getPerspectiveTransform(
        src_rect, dst_quad)  # maps warped -> original

    # --- Load path JSON and collect a warped-space polyline ---
    path_data = read_path_json(args.path_json)
    poly_warped = np.array(collect_polyline_pixels(
        path_data, mode=args.from_mode), dtype=np.float32).reshape(-1, 1, 2)
    ...
    for i in range(len(poly_orig_int) - 1):
        cv2.line(img_overlay, poly_orig_int[i], poly_orig_int[i + 1],
                 color=(0, 0, 255), thickness=args.line_width)  # red BGR
    # Mark endpoints for clarity
    if len(poly_orig_int) >= 1:
        cv2.circle(img_overlay, poly_orig_int[0], max(
            6, args.line_width * 2), (0, 255, 0), thickness=3)  # start (green)
    if len(poly_orig_int) >= 2:
        cv2.circle(img_overlay, poly_orig_int[-1], max(
            6, args.line_width * 2), (0, 0, 255), thickness=3)  # end (red)
...
def fit_affine(img_pts, rob_xy):
    ...
    if M is None:
        raise RuntimeError(
            "Affine estimation failed. Points may be degenerate.")
    return M
...
def fit_homography(img_pts, rob_xy):
    ...
    if H is None:
        raise RuntimeError(
            "Homography estimation failed. Points may be degenerate.")
    return H
...
def move_to_home(device):
    ...
    # Move to safe home position with correct coordinates
    device.move_to(240, 0, 150, 0)  # CHANGED FROM (200, 0, 50, 0)
    time.sleep(2)
    try:
...
# ---------------------affine transformation and homogeneous matrix----------------------
# YOUR CALIBRATED MATRICES from Maze_Motion.py
M = np.array([
...
# DELETE THESE LINES (lines 1347-1352):
# move_to_home(device)
# capture_image_main()
# maze_warp_from_json()
...
print("="*70)
device.speed(50, 50)
move_to_home(device)  # Return to home before starting maze
time.sleep(2)

# Initialize an empty list for coordinates in case file loading fails
pixel_coords = []
...
try:
    with open(json_path, 'r') as f:
        ...
        if 'unwarped_path_pixels' in data:
            ...
            print(
                f"Error: JSON file '{json_path}' loaded, but expected list or object with 'unwarped_path_pixels' key.")

    print(
        f"✓ Successfully loaded {len(pixel_coords)} waypoints from {json_path}")
